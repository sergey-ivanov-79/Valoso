    ...
    bootstrapSelect.prototype = {
        init: function() {
            ...
            options.each(function() {
                if ($(this).prop("disabled")) {
                    option_classname = 'class="disabled"';                    
                } else {
                    option_classname = '';
                }                
                _self.generatedSelect.find('ul').append('<li ' + option_classname + '><a tabindex="-1" href="' + $(this).val() + '">' + $(this).html() + '</a></li>');
                if ($(this).is(':selected')) {
        ...
        open: function() {
            ...
            if ( (this.settings['max-visible-items'] > 0) && (this._max_height === 0) ) { 
                ...
                this._options_container_height = this._max_height;
                               
                /* if currently active item is not in the visible are - scroll to show it */                
                var from_top = (this._currently_selected_option * this._option_height) + (this._options_container_spacing / 2);                
                if (from_top >= this._options_container_height) {
                    $(this.generatedSelect).find('ul').scrollTop( from_top - (this._options_container_spacing / 2) );
                }
            }
        ...
        disable_option: function(position) {
            var option_to_disable = $(this.generatedSelect).find('.dropdown-menu li:nth-child('+(position+1)+')');            
            if (this.get_value() === option_to_disable.find('a').attr('href') ) {
                this.set_value('');
            }
            $(this.generatedSelect).find('.dropdown-menu li:nth-child('+(position+1)+')').addClass('disabled');
            
        },
        enable_option: function(position) {
            $(this.generatedSelect).find('.dropdown-menu li:nth-child('+(position+1)+')').removeClass('disabled');
        },
        is_option_disabled: function(position) {
            return $(this.generatedSelect).find('.dropdown-menu li:nth-child('+(position+1)+')').hasClass('disabled');
        }
    };
